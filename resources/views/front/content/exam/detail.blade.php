                                ...
                                @if($questionGroup->answer_inside_content)
                                    @php
                                        ...
                                        foreach ($question as $q) {
                                            $questionMap[$q->number] = $q->id;
                                            if (isset($q->myAnswer)) {
                                                $myAnswerArr[$q->number] = $q->myAnswer->answer;
                                            } else {
                                                $myAnswerArr[$q->number] = '';
                                            }
                                        }


                                        $newContent = preg_replace_callback('/\[(\d+)\]/', function ($matches) use ($questionMap, $myAnswerArr) {
                                            $number = $matches[1];
                                            return '
                                                <span class="question-item ml-2 mr-2"  data-question="'.$questionMap[$number].'">
                                                    ...
                                                    <input type="text" name="answers[' . $number . ']" value="'.$myAnswerArr[$number].'" class="inline-block preview-input form-control form-control-sm w-32 ml-1" data-index="'.$questionMap[$number] .'"/>
                                                </span>
                                            ';
                                                ...
                                                <div class="question-item grid grid-cols-1 gap-1 mt-2" data-question="{{$question->id}}">
                                                            ...
                                                            <div class="flex justify-start items-center rounded gap-4">
                                                                <select name="answers[{{ $question->id }}]" class="preview-input form-select form-select-sm text-sm" data-index="{{$question->id}}">
                                                                    ...
                                                                    @foreach($question->answers as $answer)
                                                                        <option value="{{ $answer->value }}" {{(isset($question->myAnswer) && $question->myAnswer->answer == $answer->value) ? 'selected' : ''}}>{{ ($answer->label ?? '') .'. '. ($answer->value ?? '') }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                        </div>
                                                    @elseif ($question->input_type == 'textarea')
                                                        <div class="answer-config-item mt-4">
                                                            ...
                                                            >{{ isset($question->myAnswer) ? $question->myAnswer->answer : '' }}</textarea>
                                                    ...
                                                    @elseif ($question->input_type == 'text')
                                                        <div class="answer-config-item mt-2">
                                                            <input  type="text" name="answers[{{ $question->id }}]" class="preview-input form-control form-control-sm w-full text-base"
                                                                   data-index="{{$question->id}}" placeholder="Nhập đáp án vào chỗ trống" value="{{ isset($question->myAnswer) ? $question->myAnswer->answer : '' }}"/>
                                                        </div>
                                                    @else
                                                        @foreach($question->answers as $answer)
                                                            <div class="answer-config-item border rounded p-3 hover:bg-green-200">
                                                                    ...
                                                                    <div class="flex justify-start items-center rounded gap-4">
                                                                        @if($question->input_type == 'radio')
                                                                            <input
                                                                                ...
                                                                                value="{{ $answer->value }}"
                                                                                class="preview-input form-radio h-4 w-4 text-primary form-control"
                                                                                data-index="{{$question->id}}"
                                                                                {{(isset($question->myAnswer) && $question->myAnswer->answer == $answer->value) ? 'checked' : ''}}
                                                                            />
                                                                        @else
                                                                            @php
                                                                                $myAnswerArr = isset($question->myAnswer) ? explode('||', $question->myAnswer->answer) : [];
                                                                            @endphp
                                                                            <input
                                                                                type="checkbox"
                                                                                name="answers[{{ $question->id }}][]"
                                                                                value="{{ $answer->value }}"
                                                                                class="preview-input form-checkbox h-4 w-4 text-primary form-control"
                                                                                data-index="{{$question->id}}"
                                                                                {{ (isset($question->myAnswer) && in_array($answer->value, $myAnswerArr) ) ? 'checked' : ''}}
                                                                            />
                                                                        @endif
                                                                        @if($answer->label)
